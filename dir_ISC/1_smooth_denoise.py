...
# This one is the original Chang parcellation but a lot of it looks wrong so I'm trying this Schaefer one instead.
# mask = Brain_Data('http://neurovault.org/media/images/2099/Neurosynth%20Parcellation_0.nii.gz')

# This is the Schaefer Parcellation. We're trying the 400 parcel version with 17 networks to maximize resolution. 
...
for f in file_list:
    ...
    stats['residual'].write(os.path.join(data_dir, sub, 'func', f'{sub}_task-{task}_{run}_space-MNI152NLin2009cAsym_desc-preproc_bold_denoise_smooth{fwhm}mm_.nii.gz'))
    print("Writing " + str(sub) + "'s " + str(run) + " .hdf5 data | " + str(datetime.datetime.now()))  

    # Converting the nifti file to .hdf5 files 
    stats['residual'].write(os.path.join(data_dir, sub, 'func', f'{sub}_task-{task}_{run}_space-MNI152NLin2009cAsym_desc-preproc_bold_denoise_smooth{fwhm}mm_.hdf5'))
    ...
    pd.DataFrame(roi.T).to_csv(os.path.join(os.path.dirname(f), f"{sub}_{run}_Average_ROI_n400.csv" ), index=False)
