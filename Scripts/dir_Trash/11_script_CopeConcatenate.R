...
# Reading In Data
{
  ...
  for (FILE in files){
    ...
    if (file.size(paste0(Raw, FILE)) > 5){
      ...
      x <- paste0("X", df_temp[,1])
      y <- paste0("Y", df_temp[,2])
      z <- paste0("Z", df_temp[,3])
    ...
    if (progress == 1 | progress %% 1000 == 0){
      ...
      paste0("===> sub-", PID, " || ", percent, "% Completed at ", Sys.time(), " || Estimated Time Remaining: ", remaining_time, " minutes") %>%
        print()
    }
  }
}
